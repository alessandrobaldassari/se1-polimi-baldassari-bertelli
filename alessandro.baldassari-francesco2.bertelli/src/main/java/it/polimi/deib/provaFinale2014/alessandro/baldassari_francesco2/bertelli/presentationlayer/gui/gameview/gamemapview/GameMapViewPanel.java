...
public class GameMapViewPanel extends ObservableFrameworkedWithGridBagLayoutPanel < GameMapViewObserver > implements GameMapObserver 
{
	...
	{
		//positionableElementsManager.addElement ( GUIGameMapNotificationMessage.newInstance ( "ADD" , whereType , whereId , whoType , whoId ) ) ;
		//repaint () ;
		animator.addAddMessage ( GUIGameMapNotificationMessage.newInstance ( "ADD" , whereType , whereId , whoType , whoId ) );
	}
	...
	{
		//positionableElementsManager.removeElement ( GUIGameMapNotificationMessage.newInstance ( "REMOVE" , whereType , whereId , whoType , whoId ) ) ;
		//repaint () ;
		animator.addRemoveMessage ( GUIGameMapNotificationMessage.newInstance( "REMOVE" , whereType , whereId , whoType , whoId ) );
	}
	...
	private class DrawingPanel extends JPanel 
	{
		...
		private void drawRegionsElement ( Graphics g ) 
		{
			...
			for ( i = 1 ; i <= GameConstants.NUMBER_OF_REGIONS ; i ++ )
			{
				...
				{
					toDraw =  SheeplandClientApp.getInstance().getImagesHolder().getPositionableImage ( PositionableElementType.BLACK_SHEEP , transparent , false ) ;
				...
				{
					toDraw =  SheeplandClientApp.getInstance().getImagesHolder().getPositionableImage ( PositionableElementType.WOLF , transparent , false ) ;
				...
				if ( count > 0 )
				{
					toDraw =  SheeplandClientApp.getInstance().getImagesHolder().getPositionableImage ( PositionableElementType.SHEEP , transparent , false ) ;
		...
		private void drawRoadsElements ( Graphics g ) 
		{
			...
			for ( i = 1 ; i <= GameConstants.NUMBER_OF_ROADS ; i ++ )
			{
				...
				if ( elementContained != null ) 
				{
					...
					toDraw = SheeplandClientApp.getInstance().getImagesHolder().getPositionableImage ( type , transparent , false ) ;
		...
		private void drawRegionsHighlighted ( Graphics g ) 
		{
			...
			if ( currentInputMode != null && currentInputMode == GameMapViewInputMode.REGIONS )
				for ( Identifiable i : uidOfSelectableElements )
				{
					...
					p = coordinatesManager.getRegionBorder(i.getUID());
					notScaled = GraphicsUtilities.approximateExternSimple(p);
					r = scaleCoordinates ( notScaled.x , notScaled.y , notScaled.width , notScaled.height ) ;
					g.drawImage ( toDraw , r.x , r.y , r.width , r.height , this ) ;
				}
		}
		...
		public void drawRoadHighlighted ( Graphics g ) 
		{
			...
			Ellipse2D e ;
			Rectangle r ;
			if ( currentInputMode != null && currentInputMode == GameMapViewInputMode.ROADS )
			{
				...
				if ( currentInputMode != null )
				{
					...
					switch ( currentInputMode )
					{
						...
						case SHEPERDS :
							...
							i = coordinatesManager.getRoadId ( r.x , r.y ) ;
							couple = positionableElementsManager.getElementInRoad ( i ) ;
							if ( couple != null && PositionableElementType.isSheperd  ( couple.getFirstObject() ) )
								uid = couple.getSecondObject();
							else
								uid = null ;
